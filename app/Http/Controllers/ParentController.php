...
class ParentController extends Controller
{
    ...
    {
        // dd(1);
        //Lấy ra tất cả các khóa học đã đăng ký của các học sinh có phụ huynh đang đăng nhập
        $id = Auth::user()->id;
        $registeredCourses = DB::table('student_classes')
        ->join('classes', 'student_classes.class_id', '=', 'classes.id')
        ->join('courses', 'classes.course_id', '=', 'courses.id')
        ->join('students', 'students.user_id', '=', 'student_classes.student_id')
    ...
    ->groupBy('student_name');
        // dd($registeredCourses->all());
        return view('user.parent.registered-courses', compact('registeredCourses'));
    }
    ...
    {
        //  dd(2);
        // Lấy ra tất cả các lớp học đã đăng ký của các học sinh có phụ huynh đang đăng nhập
        $id = Auth::user()->id;
    ...
    ->groupBy('student_name');
        ...
        return view('user.parent.registered-class', compact('registeredClass'));
    }
...
public function schedule(Request $request)
{
    ...
    // Get schedules for the selected child or first child by default
    $selectedChildId = $request->input('child_id', $children->first()->id ?? null);
    ...
    $schedules = DB::table('schedules as s')
        ...
        ->where('st.user_id', $selectedChildId)
        ->when($request->filled('from_date'), function($query) use ($request) {
            return $query->where('s.start_date', '>=', $request->from_date);
        })
        ->when($request->filled('to_date'), function($query) use ($request) {
            return $query->where('s.start_date', '<=', $request->to_date);
        })
        ->groupBy(
            's.id',
            's.start_date',
    ...
    return view('user.parent.schedules', compact('schedules', 'children', 'selectedChildId'));
}
